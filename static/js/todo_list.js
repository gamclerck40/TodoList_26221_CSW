...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ======================================================
  // 0-3) 현재 로그인 사용자명 추정
  // - localStorage 등에 저장된 username을 우선 사용
  // - 프로젝트마다 저장 키가 다를 수 있어 후보 키를 순서대로 확인
  // ======================================================
  ...
  // [추가된 헬퍼]
  // 내 글 여부 판단
  // - 현재 로그인 사용자명과 todo.username 비교
  // ======================================================
  function isMyTodo(todo) {
    if (!currentUsername) return false;
    return (todo.username ?? "") === currentUsername;
  }
  ...
  function renderTodos(todos) {
    ...
    todos.forEach((todo) => {
      ...
      // ======================================================
      // [추가된 코드]
      // 내 글 / 다른 사용자 글 여부 표시용 값
      // ======================================================
      const mine = isMyTodo(todo);
      const authorLabel = mine ? "(내 글)" : "(다른 사용자 글)";
      ...
      card.innerHTML = `
        ...
        <p>
          <strong>작성자:</strong> ${todo.username ?? ""}
          <span style="margin-left:8px; font-size:13px; color:${mine ? "green" : "#666"};">
            ${authorLabel}
          </span>
        </p>
